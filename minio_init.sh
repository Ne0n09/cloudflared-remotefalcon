...
container_running() {
  ...
  local running
  running=$(docker inspect -f '{{.State.Running}}' "$container_name" 2>/dev/null) || return 1
  [[ "$running" == "true" ]]
}
...
if [[ $changed_creds == true ]]; then
  ...
  docker compose -f "$COMPOSE_FILE" rm -f -s minio
  docker compose -f "$COMPOSE_FILE" up -d minio
  check_minio_health
else
...
while [[ $retry_count -lt $max_retries ]]; do
  output=$(docker exec "$CONTAINER_NAME" $ALIAS_CMD 2>&1) && alias_success=true && break
...
if docker exec "$CONTAINER_NAME" mc ls "$MINIO_ALIAS" | grep -q "$BUCKET_NAME/"; then
  echo -e "${GREEN}✅ Bucket '$BUCKET_NAME' already exists.${NC}"
else
  echo "🪣 Creating bucket '$BUCKET_NAME'..."
  docker exec "$CONTAINER_NAME" mc mb "$MINIO_ALIAS/$BUCKET_NAME"
fi

# Make bucket public
if docker exec "$CONTAINER_NAME" mc anonymous get "$MINIO_ALIAS/$BUCKET_NAME" | grep -q "Access permission.*is.*public"; then
  echo -e "${GREEN}✅ Bucket '$BUCKET_NAME' is already public.${NC}"
else
  echo "🪣 Making bucket '$BUCKET_NAME' public..."
  docker exec "$CONTAINER_NAME" mc anonymous set public "$MINIO_ALIAS/$BUCKET_NAME"
fi
...
if [[ $S3_ACCESS_KEY == "123456" || $S3_SECRET_KEY == "123456" ]]; then
  ...
  access_output=$(docker exec "$CONTAINER_NAME" mc admin accesskey create --json $MINIO_ALIAS --name $ACCESS_KEY_NAME)
