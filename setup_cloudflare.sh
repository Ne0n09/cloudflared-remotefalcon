...
if [ ! -f "$SCRIPT_DIR/shared_functions.sh" ]; then
  echo -e "$${YELLOW}⚠️ shared_functions.sh not found. Downloading shared_function.sh...${NC}"
  mkdir -p remotefalcon; curl -o shared_functions.sh https://raw.githubusercontent.com/Ne0n09/cloudflared-remotefalcon/refs/heads/main/shared_functions.sh
...
# Paths
#ENV_FILE="./remotefalcon/.env" # Defined in shared_functions.sh
CERT_DIR=$WORKING_DIR
...
check_requirements() {
  ...
  if [ ${#missing_tools[@]} -ne 0 ]; then
    ...
    echo -e "${YELLOW}⚠️ Installing them with: sudo apt-get install ${missing_tools[*]}${NC}"
    sudo apt-get update && sudo apt-get install ${missing_tools[*]} -y
  fi
...
read_env_file() {
  ...
  # If DOMAIN is not alrady set  attempt to read it from .env
  if [[ ! -n "${DOMAIN:-}" ]]; then
  ...
  if [ -z "${DOMAIN}" ] || [ "${DOMAIN}" = "your_domain.com" ]; then
    ...
    if [[ "$NON_INTERACTIVE" == false ]]; then
      ...
      echo -e "${RED}❌ DOMAIN must be provided with --domain in non-interactive mode.${NC}"
      exit 1
    fi
...
update_domain() {
  local domain=$1
  echo -e "${BLUE}🔄 Updating DOMAIN in ${ENV_FILE}...${NC}"
...
add_domain_to_cloudflare() {
  echo -e "${BLUE}🔍 Checking if domain '${DOMAIN}' exists in Cloudflare...${NC}"
  ...
  if [ "$ZONE_COUNT" -gt 0 ]; then
    ...
    if echo "$ADD_ZONE_RESPONSE" | jq -e '.success' > /dev/null; then
      ...
      if [[ "$NON_INTERACTIVE" == false ]]; then
        ...
        echo -e "${YELLOW}⚠️ Non-interactive mode. Please ensure nameservers are updated at your registrar!${NC}"
      fi
    else
...
#  COMPOSE_FILE="./remotefalcon/compose.yaml" # Removed as it's defined in shared_functions.sh
  ...
  if [ "$TUNNEL_COUNT" -gt 0 ]; then
    ...
    if [[ "$RECREATE" =~ ^[Yy]$ ]]; then
      if is_container_running "cloudflared"; then #is_container_running is defined in shared_functions.sh
        ...
        docker compose -f "$COMPOSE_FILE" down cloudflared 2>/dev/null && CLOUDFLARED_WAS_STOPPED=true
        sleep 3
      fi
  ...
  if echo "$TUNNEL_RESPONSE" | jq -e '.success' > /dev/null; then
    ...
    if [ "$CLOUDFLARED_WAS_STOPPED" = true ] && [ -f "$COMPOSE_FILE" ]; then
      docker compose -f "$COMPOSE_FILE" up -d cloudflared 2>/dev/null
...
main() {
  ...
  get_account_id
  add_domain_to_cloudflare
  echo -e "${BLUE}⚙️ Starting Cloudflare configuration...${NC}"
  delete_conflicting_records
  configure_ssl
  create_origin_certificate
  create_tunnel
