...
is_container_running() {
  local service="$1"
  docker compose -f "$COMPOSE_FILE" ps --services --filter "status=running" | grep -q "^$service$"
}
...
check_endpoint() {
        ...
        if [[ "$http_code" == 200 ]]; then
          ...
          if [[ $attempt -lt $MAX_RETRIES ]]; then
            ...
            if [[ "$http_code" != 200 ]]; then
              ...
              echo -e "  ${YELLOW}•${NC} ${RED}❌ $rf_container endpoint ${BLUE}🔗 $endpoint${NC} ${RED}status is NOT UP (Current status: $health_status)${NC}"
              echo -e "${YELLOW}⚠️ Check the logs with 'docker logs $rf_container' for more information.${NC}"
              HEALTHY=false
            fi
  ...
  if is_container_running $container_name; then
    echo -e "${CYAN}🔄 $container_name is running. Testing the configuration with 'docker exec $container_name nginx -t'...${NC}"
    nginx_test_output=$(docker exec $container_name nginx -t 2>&1)
  ...
  if is_container_running $container_name; then
    ...
    output=$(docker exec "$container_name" wget -qO- http://127.0.0.1:7070/health 2>/dev/null || true)
  ...
  if [[ $HEALTHY == true ]]; then
    ...
    echo -e "${YELLOW}⚠️ Check logs with 'docker logs <container_name>' or try 'docker compose -f ""$COMPOSE_FILE"" down' and 'docker compose -f ""$COMPOSE_FILE"" up -d'${NC}"
  fi
else
