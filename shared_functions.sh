...
display_env_value() {
  local key="$1" value="$2"
  case "$key" in
    *TOKEN*|*PAT*|*PASSWORD*|*SECRET*|*PRIVATE*|*JWT*|*KEY*) printf '[redacted]' ;;
    *) printf '%s' "$value" ;;
  esac
}
...
backup_file() {
  ...
  (umask 077; cp "$file_path" "$BACKUP_DIR/$filename.backup-$timestamp")
  chmod 600 "$BACKUP_DIR/$filename.backup-$timestamp"
...
backup_mongo() {
  ...
  until docker exec "$container_name" mongosh \
    --quiet \
  ...
  if ! docker cp \
    ...
    docker exec "$container_name" rm -f /tmp/backup.archive >/dev/null 2>&1
...
validate_docker_user() {
  ...
  echo "$GITHUB_PAT" | docker login ghcr.io -u "$GH_USER" --password-stdin >/dev/null 2>&1
  if [[ $? -ne 0 ]]; then
...
select_dockerfile_for_host() {
  ...
  if [[ "$DOCKERFILE" != "Dockerfile" && "$DOCKERFILE" != "Dockerfile.dev" ]]; then
    echo -e "${YELLOW}⚠️ Invalid DOCKERFILE value '$DOCKERFILE'. Resetting to Dockerfile.dev.${NC}"
...
get_current_version() {
  ...
  case "$service_name" in
    plugins-api|control-panel|viewer|ui|external-api)
      docker ps --filter "name=$(get_container_name "$service_name")" --format '{{.Image}}' | sed -E 's|^.*:||'
      ;;
    cloudflared)
      docker exec "$(get_container_name "$service_name")" cloudflared --version | sed -n 's/^cloudflared version \([0-9.]*\).*/\1/p'
      ;;
    nginx)
      docker exec "$(get_container_name "$service_name")" nginx -v 2>&1 | sed -n 's/^nginx version: nginx\///p'
      ;;
    mongo)
      docker exec "$(get_container_name "$service_name")" bash -c "mongod --version | grep -oP 'db version v\\K[\\d\\.]+'" | tr -d '[:space:]'
      ;;
    versitygw)
      docker exec "$(get_container_name "$service_name")" versitygw --version | awk '/^Version/ {print $3}'
      ;;
    *)
...
get_container_network() {
  docker inspect -f '{{range $k, $v := .NetworkSettings.Networks}}{{println $k}}{{end}}' "$1" | head -n1
}
...
check_bucket_policy() {
  ...
  policy_json=$(docker run --rm --network "container:$container_name" -e AWS_ACCESS_KEY_ID="$S3_ROOT_USER" -e AWS_SECRET_ACCESS_KEY="$S3_ROOT_PASSWORD" amazon/aws-cli --endpoint-url http://$container_name:7070 s3api get-bucket-policy --bucket "$IMAGES_S3_BUCKET" 2>/dev/null)
